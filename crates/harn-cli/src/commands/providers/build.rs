...
";
const STDLIB_PROVIDER_NAMES_GENERATED_HEADER: &str = "\
// @generated by `harn provider catalog generate`; do not edit directly.
// Edit crates/harn-vm/src/llm/catalog_sources/**/*.toml instead.
";
...
pub(crate) fn run_generate(args: &ProvidersGenerateArgs) -> Result<(), String> {
    ...
    let stdlib_provider_names = generated_stdlib_provider_names(&provider_config.config);
    ...
    if args.check {
        ...
        if file_drifted(&args.stdlib_provider_names_output, &stdlib_provider_names) {
            drift.push(args.stdlib_provider_names_output.display().to_string());
        }
    ...
    write_generated_file(&args.stdlib_provider_names_output, &stdlib_provider_names)?;
    println!("wrote {}", args.stdlib_provider_names_output.display());
...
fn generated_stdlib_provider_names(config: &harn_vm::llm_config::ProvidersConfig) -> String {
    let mut body = String::from(STDLIB_PROVIDER_NAMES_GENERATED_HEADER);
    body.push_str("/** Canonical provider names declared by the embedded provider catalog. */\n");
    body.push_str("pub const PROVIDER_NAMES: list<string> = [\n");
...
mod tests {
    ...
    use super::{
        generated_provider_config, generated_stdlib_provider_names, leading_bare_key_error,
        run_generate,
    };
...
"#,
    ...
    #[test]
    fn stdlib_provider_names_are_a_sorted_projection_of_the_catalog() {
        let config = harn_vm::llm_config::parse_config_toml(
            r#"
[providers.zeta]
...
"#,
        ...
        let generated = generated_stdlib_provider_names(&config);
        assert!(generated.contains("@generated by `harn provider catalog generate`"));
        assert!(
            generated.find("\"alpha\"").unwrap() < generated.find("\"zeta\"").unwrap(),
            "BTreeMap provider order should make the projection deterministic"
        );
    }
...
"#,
        ...
        let args = ProvidersGenerateArgs {
            ...
            stdlib_provider_names_output: tempdir.path().join("out/provider_names.harn"),
            check: false,
        };
        ...
        let stdlib_names =
            fs::read_to_string(&args.stdlib_provider_names_output).expect("stdlib provider names");
        assert!(stdlib_names.contains("\"private\""));
