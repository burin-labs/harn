//! Orchestrator-server stable compatibility tests.
//!
//! Drives the shared `harn-mcp-compat` fixtures + fake-client helpers
//! against the in-process `McpOrchestratorService`. Failures here
//! localize to the **orchestrator-server** surface — see
//! `crates/harn-mcp-compat/tests/generic_server.rs` for the parallel
...
use harn_mcp_compat::fake_client::{stable_meta, stable_request};
...
async fn orchestrator_stable_tools_list_carries_envelope_and_cache_hint() {
    let _guard = lock_harn_state_async().await;
    let (service, _temp) = fresh_service().await;
    let mut session = ConnectionState::default();
    let request = stable_request(1, "tools/list", json!({}), "harn-mcp-compat-client");
    ...
    assert!(
        ...
        "stable tools/list must include cache hint"
    );
    assert_eq!(result["cacheScope"], json!("private"));
}

#[tokio::test(flavor = "current_thread")]
async fn orchestrator_stable_ping_carries_result_type_envelope() {
    let _guard = lock_harn_state_async().await;
    let (service, _temp) = fresh_service().await;
    let mut session = ConnectionState::default();
    let request = stable_request(1, "ping", json!({}), "harn-mcp-compat-client");
    let response = service.handle_request(&mut session, request).await;
    let result = response.get("result").expect("ping result");
    assert_eq!(result["resultType"], json!("complete"));
}

#[tokio::test(flavor = "current_thread")]
async fn orchestrator_unsupported_meta_version_returns_minus_32022() {
    let _guard = lock_harn_state_async().await;
    let (service, _temp) = fresh_service().await;
    let mut session = ConnectionState::default();
    let request = json!({
...
#[tokio::test(flavor = "current_thread")]
async fn orchestrator_server_discover_returns_stable_capabilities() {
    let _guard = lock_harn_state_async().await;
    let (service, _temp) = fresh_service().await;
    let mut session = ConnectionState::default();
    let request = json!({
...
#[tokio::test(flavor = "current_thread")]
async fn orchestrator_replays_published_stable_success_fixture() {
    let _guard = lock_harn_state_async().await;
    let (service, _temp) = fresh_service().await;
    ...
    for pair in fixture.documents.chunks(2) {
        ...
        // list still validate the orchestrator's stable envelope/cache hint
