...
#[test]
fn unknown_type_selectors_apply_fail_fast_validation_and_clear_the_errors() {
    let temp = tempfile::TempDir::new().unwrap();
    let script = temp.path().join("boundary.harn");
    fs::write(
        &script,
        concat!(
            "import { schema_expect } from \"std/schema\"\n",
            "fn consume(value: string) -> string { return value }\n",
            "fn cross_boundary(value: unknown) -> string { return consume(value) }\n",
            "fn typed_return(value: unknown) -> string { return value }\n",
            "fn with_default(value: unknown) -> string { return consume(value ?? \"\") }\n",
        ),
    )
    .unwrap();
    let options = FixOptions {
        codes: BTreeSet::from([Code::ArgumentTypeMismatch, Code::ReturnTypeMismatch]),
        ..FixOptions::default()
    };

    let result =
        apply_repairs_with_options_at(&script, RepairSafety::ScopeLocal, false, options).unwrap();

    assert_eq!(result.applied.len(), 3, "{result:#?}");
    assert_eq!(
        fs::read_to_string(&script).unwrap(),
        concat!(
            "import { schema_expect } from \"std/schema\"\n",
            "fn consume(value: string) -> string { return value }\n",
            "fn cross_boundary(value: unknown) -> string { return consume(",
            "schema_expect(value, {type: \"string\"})) }\n",
            "fn typed_return(value: unknown) -> string { return ",
            "schema_expect(value, {type: \"string\"}) }\n",
            "fn with_default(value: unknown) -> string { return consume(",
            "schema_expect(value, {type: \"string\"})) }\n",
        )
    );
}
