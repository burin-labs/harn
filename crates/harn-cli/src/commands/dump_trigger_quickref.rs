...
fn generate_file() -> String {
    ...
    out.push_str("Run `harn package verify .` locally. Add `--strict` for warning-fatal check/lint gates and strict boundary typing, use `--provider <id>` for a multi-provider package, `--run-poll-tick` to execute the first poll tick, and `--json` for a schema-v3 CI receipt.\n\n");
...
the connector that consumes the topic.\n\n",
    ...
    out.push_str("Runtime capability methods, reached through `harness.runtime`:\n\n");
    out.push_str(
