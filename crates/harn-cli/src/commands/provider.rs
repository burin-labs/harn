...
use crate::cli::{
    ProviderCacheProbeArgs, ProviderEffortProbeArgs, ProviderProbeArgs, ProviderToolProbeArgs,
...
async fn aggregate_provider_probe(args: &ProviderProbeArgs) -> ProviderProbe {
    ...
    ProviderProbe {
        ...
        base_url,
        readiness,
        runtime_profile: local_runtime_for_provider(&args.provider).and_then(|_| {
...
/// `harn provider effort-probe` — thin argv shim; the probe itself is
/// `stdlib/cli/providers/effort_probe.harn`.
///
/// The only thing this shim decides is whether Harn's declared-ladder check
/// stays on. It is suspended by default: the check refuses an out-of-ladder
/// effort before the request leaves the process, so with it on the probe could
/// only ever re-measure the catalog against itself and no drift would be
/// findable. `--gated` restores it for callers who want a confirm-only run.
pub(crate) async fn run_provider_effort_probe(args: ProviderEffortProbeArgs) -> i32 {
