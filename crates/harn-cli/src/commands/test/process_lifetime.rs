...
pub(super) fn surviving_helpers_error() -> Option<String> {
    let initial_survivors = harn_vm::op_interrupt::current_process_owner_survivors();
    ...
    if std::env::var_os("CI").is_some() {
        ...
        #[cfg(not(unix))]
        let current_group = -1;
        let candidates = initial_survivors
            .iter()
            .map(|process| {
                #[cfg(unix)]
            ...
            .join(", ");
        eprintln!(
            "conformance helper lifetime cleanup candidates: runner_pid={current_pid} runner_pgid={current_group}; {candidates}"
        );
