...
pub fn verify(args: &PackVerifyArgs) -> Result<PackVerifyJsonData, PackError> {
    ...
    if args.strict {
        verify_sbom_package_hashes(manifest, &archive_hashes, SbomHashScope::Strict)?;
    }
...
pub(crate) fn verify_runtime_payloads(
    ...
    verify_sbom_package_hashes(manifest, &archive_hashes, SbomHashScope::RuntimePaths)?;
    ...
    let archived_sbom: harn_vm::orchestration::SBOMDoc = serde_json::from_slice(&sbom_entry.bytes)
        .map_err(|error| {
            PackError::new(
                "verify.sbom_mismatch",
                format!("archive SBOM is not valid JSON: {error}"),
            )
        })?;
    if archived_sbom != manifest.sbom {
        return Err(PackError::new(
            "verify.sbom_mismatch",
            format!(
...
enum SbomHashScope {
    /// Bind payloads whose runtime identity comes from the SBOM. Module source
    /// identity is already authoritative in `transitive_modules`.
    RuntimePaths,
    /// Also audit redundant module and package metadata for standalone strict
    /// verification.
    Strict,
}
...
fn verify_sbom_package_hashes(
    ...
    for package in &manifest.sbom.packages {
        ...
        if let Some(rel) = package.name.strip_prefix("module:") {
            if scope == SbomHashScope::RuntimePaths {
                continue;
            }
