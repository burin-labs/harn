...
pub(crate) fn run_list(args: &GuardListArgs) {
    let store = store();
    let mut installed = store.installed(ModelPurpose::InjectionClassification);
    installed.extend(store.installed(ModelPurpose::Embedding));
...
pub(crate) fn run_status(args: &GuardStatusArgs) {
    ...
    let names: Vec<String> = match &args.model {
        ...
        None => {
            let mut names = store.installed(ModelPurpose::InjectionClassification);
            names.extend(store.installed(ModelPurpose::Embedding));
            names.sort_by(|a, b| a.name.cmp(&b.name));
            names.into_iter().map(|m| m.name).collect()
        }
    };
    if names.is_empty() {
...
pub(crate) async fn run_install(args: &GuardInstallArgs) {
    ...
    let Some(model) = catalog::find_by_name(&name) else {
    ...
    if store.is_installed(model.name, model.purpose) && !args.force {
        println!(
