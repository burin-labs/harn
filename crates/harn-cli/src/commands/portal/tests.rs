...
use super::transcript::{discover_template_renders, discover_transcript_steps};
...
#[test]
fn discover_template_renders_joins_snapshots_and_preserves_legacy_inline_events() {
    let temp = tempfile::tempdir().unwrap();
    let run_path = temp.path().join("run.json");
    fs::write(&run_path, "{}").unwrap();
    let llm_dir = temp.path().join("run-llm");
...
#[test]
fn discover_template_renders_rejects_a_snapshot_body_that_mismatches_its_id() {
    let temp = tempfile::tempdir().unwrap();
