...
mod tests {
    ...
    #[test]
    fn validate_refuses_a_retired_limits_section() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("harn.config.toml");
        ...
        assert!(
            error.contains("limits"),
            "validate must name the retired section, got: {error}"
        );
    }
}
