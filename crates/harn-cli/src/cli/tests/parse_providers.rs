...
#[test]
fn test_providers_overlay_audit_requires_an_overlay() {
    // The audit is meaningless without a file to audit, so `--overlay` is a
    // required arg rather than a defaulted one.
