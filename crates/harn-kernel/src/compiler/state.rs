...
impl Compiler {
    ...
    /// module.
    #[doc(hidden)]
    pub fn prepare_module_context(&mut self, program: &[SNode]) {
    ...
    /// canonical `TypeExpr`.
    #[doc(hidden)]
    pub fn collect_type_aliases(&mut self, program: &[SNode]) {
        for sn in program {
            match peel_node(sn) {
    ...
    #[doc(hidden)]
    pub fn expand_alias(&self, ty: &TypeExpr) -> TypeExpr {
    ...
    #[doc(hidden)]
    pub fn type_expr_to_schema_value(type_expr: &harn_parser::TypeExpr) -> Option<VmValue> {
        match type_expr {
    ...
    pub(super) fn compile_block(&mut self, stmts: &[SNode]) -> Result<(), CompileError> {
        self.record_monomorphic_var_bindings(stmts);
        let callable_declarations = stmts
            .iter()
            .enumerate()
            .filter_map(|(index, node)| {
                harn_parser::lexical::hoisted_callable_name(node)
                    .map(|name| (index, name.to_string()))
            })
            .collect::<Vec<_>>();
        let callable_names = callable_declarations
            .iter()
            .map(|(_, name)| name.as_str())
            .collect::<std::collections::HashSet<_>>();
        let mut emitted_callables = std::collections::HashSet::new();

        for (i, snode) in stmts.iter().enumerate() {
            if harn_parser::lexical::hoisted_callable_name(snode).is_some() {
                if emitted_callables.insert(i) {
            ...
            // Function-like declarations are visible throughout their block.
            // Materialize only the forward declarations reachable from this
            // statement (including mutual-recursion dependencies), at the
            // latest safe point. Earlier data bindings have therefore run and
            // remain available to the closure, while a forward call cannot
            // reach an uninitialized callable slot.
            let mut pending = Vec::new();
            Self::collect_callable_references(snode, &callable_names, &mut pending);
            let mut reachable = std::collections::HashSet::new();
            while let Some(name) = pending.pop() {
                if !reachable.insert(name.clone()) {
                    continue;
                }
                for (index, declaration_name) in &callable_declarations {
                    if declaration_name == &name {
                        Self::collect_callable_references(
                            &stmts[*index],
                            &callable_names,
                            &mut pending,
                        );
            ...
            for (index, name) in &callable_declarations {
                if reachable.contains(name) && emitted_callables.insert(*index) {
                    self.compile_discarded_stmt(&stmts[*index])?;
                }
            }
    ...
    fn collect_callable_references(
        ...
        match &node.node {
            Node::Identifier(name) | Node::FunctionCall { name, .. }
        ...
        for child in harn_parser::visit::immediate_children(node) {
            Self::collect_callable_references(child, callable_names, out);
        }
    }
