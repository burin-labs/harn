...
#[test]
fn named_entry_bootstrap_defines_sibling_functions() {
    ...
    let entry = Compiler::new()
        ...
        .unwrap();
    let bootstrap = entry.bootstrap.disassemble("entry bootstrap");
    assert!(
        bootstrap.contains("DEF_LET") && bootstrap.contains("collect"),
        "sibling function is absent from named entry bootstrap:\n{bootstrap}"
    );
    assert!(
        bootstrap.contains("reduce"),
        "target function is absent from named entry bootstrap:\n{bootstrap}"
    );
    let reduce = entry
        .bootstrap
        .functions
    ...
    assert!(
        reduce.chunk.disassemble("reduce").contains("collect"),
        "sibling call was not preserved in the target closure"
    );
}
...
fn oversized_function_chunk_is_a_compile_error_not_a_miscompile() {
    ...
    let source = format!(
        "fn huge(task: int) {{\n{body}}}\npipeline t(harness: Harness, task: unknown) {{ huge(1) }}"
    );
...
fn match_list_pattern_rest_must_be_last() {
    let err = try_compile(
        r"pipeline t(harness: Harness, task: unknown) { match [1, 2, 3] { [...rest, last] -> { harness.stdio.log(last) } _ -> {} } }",
    )
    .unwrap_err();
...
fn match_list_pattern_rejects_two_rests() {
    let err = try_compile(
        r"pipeline t(harness: Harness, task: unknown) { match [1, 2, 3] { [a, ...x, ...y] -> { harness.stdio.log(a) } _ -> {} } }",
    )
    .unwrap_err();
    assert!(
...
fn test_compile_arithmetic() {
    let chunk = compile_source_with_options(
        "pipeline test(harness: Harness, task: unknown) { const x = 2 + 3 }",
...
fn test_optimizer_folds_scalar_constants() {
    let chunk = compile_source(
        "pipeline test(harness: Harness, task: unknown) { harness.stdio.log(2 + 3 * 4) }",
    );
    let disasm = chunk.disassemble("test");
...
fn test_optimizer_keeps_runtime_erroring_arithmetic_unfolded() {
    let chunk = compile_source(
        "pipeline test(harness: Harness, task: unknown) { harness.stdio.log(1 / 0) }",
    );
    let disasm = chunk.disassemble("test");
    let opcodes = disasm_opcodes(&disasm);
...
fn test_optimizer_keeps_large_allocations_unfolded() {
    let chunk = compile_source(
        r#"pipeline test(harness: Harness, task: unknown) { harness.stdio.log("x" * 1000000) }"#,
    );
    let disasm = chunk.disassemble("test");
    let opcodes = disasm_opcodes(&disasm);
...
fn test_compile_function_call() {
    let chunk =
        compile_source("pipeline test(harness: Harness, task: unknown) { harness.stdio.log(42) }");
    let disasm = chunk.disassemble("test");
...
fn test_compile_if_else() {
    let chunk = compile_source(
        r#"pipeline test(harness: Harness, task: unknown) { if true { harness.stdio.log("yes") } else { harness.stdio.log("no") } }"#,
    );
    let disasm = chunk.disassemble("test");
...
fn test_compile_while() {
    let chunk = compile_source(
        "pipeline test(harness: Harness, task: unknown) { let i = 0\n while i < 5 { i = i + 1 } }",
    );
    let disasm = chunk.disassemble("test");
    assert!(disasm.contains("JUMP_IF_FALSE"));
    assert!(disasm.contains("JUMP"));
...
fn test_compile_closure() {
    let chunk = compile_source(
        "pipeline test(harness: Harness, task: unknown) { const f = { x -> x * 2 } }",
    );
...
fn test_compile_list() {
    let chunk =
        compile_source("pipeline test(harness: Harness, task: unknown) { const a = [1, 2, 3] }");
    let disasm = chunk.disassemble("test");
...
fn test_compile_dict() {
    let chunk = compile_source(
        r#"pipeline test(harness: Harness, task: unknown) { const d = {name: "test"} }"#,
    );
    let disasm = chunk.disassemble("test");
...
fn test_disassemble() {
    let chunk = compile_source_with_options(
        "pipeline test(harness: Harness, task: unknown) { harness.stdio.log(2 + 3) }",
        CompilerOptions::without_optimizations(),
    );
    let disasm = chunk.disassemble("test");
...
fn return_call_inside_handler_does_not_tail_call() {
    let chunk = compile_source(
        r"
fn plain<T>(body: fn() -> T) -> T {
  return body()
}

fn guarded<T>(body: fn() -> T) -> T? {
  try {
    return body()
  } catch (e) {
    return nil
...
fn guarded_retry<T>(body: fn() -> T) -> T {
  retry(1) {
    return body()
  }
}
