...
/// `llm_stream_call`, and friends. Backed directly by the canonical option
/// registry in [`crate::llm_options`] — the same table the runtime unknown-key
/// gate, the stdlib allowlist builtin, and the lint rules consume. Add or
/// remove keys THERE, never here.
pub const LLM_CALL_OPTIONS: Ty = Ty::Shape(crate::llm_options::LLM_CALL_OPTION_FIELDS);
...
pub const LLM_USAGE: Ty = Ty::Shape(&[
    ...
    ShapeFieldDescriptor::new("unpriced_tokens", TY_INT),
    ShapeFieldDescriptor::new("unpriced_reason", TY_STRING_OR_NIL),
