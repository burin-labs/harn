...
pub(crate) fn builtin_doc(name: &str) -> Option<String> {
    let doc = match name {
        ...
        "render" => "**render(path, bindings?)** → string — Render a source-relative asset with optional `${name}` bindings",
        "render_prompt" => "**render_prompt(path, bindings?)** → string — Prompt-oriented alias of `render(...)` that marks `.prompt` assets explicitly in bundle contracts",
