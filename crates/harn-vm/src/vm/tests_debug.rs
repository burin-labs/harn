...
fn test_evaluate_in_frame_literal() {
    ...
    let chunk =
        crate::compile_source("const __seed__: int = 0\nconst __sink__ = __seed__\n").unwrap();
...
fn test_restart_frame_rewinds_ip_and_rebinds_args() {
    ...
    vm.set_breakpoints(vec![3]);
    let chunk = crate::compile_source(
        "fn inner(n: int) -> int { \n  const doubled: int = n * 2\n  const __sink__ = doubled\n  return doubled\n}\nconst __result__ = inner(21)\n",
    )
    .unwrap();
    run_until_paused(&mut vm, &chunk);
...
fn test_evaluate_in_frame_parse_error_is_surfaced_standalone() {
    ...
    let chunk = crate::compile_source("const __sink__ = 0\n").unwrap();
    run_until_paused(&mut vm, &chunk);
