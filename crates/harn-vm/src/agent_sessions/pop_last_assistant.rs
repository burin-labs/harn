...
pub fn pop_last_if_assistant(id: &str) -> Result<bool, String> {
    let removed = super::SESSIONS.with(|sessions| {
        ...
        Ok(Some((source_event_id, trailing.clone())))
    })?;
    if let Some((source_event_id, raw_message)) = removed {
        crate::agent_session_journal::enqueue_message_removed(
            id,
            source_event_id.unwrap_or_else(|| uuid::Uuid::now_v7().to_string()),
            crate::llm::helpers::vm_value_to_json(&raw_message),
        );
    }
    Ok(true)
}
