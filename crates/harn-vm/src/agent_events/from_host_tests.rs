...
// ---------------------------------------------------------------------------
// harn#8217 step 3: the emitters the census named.
//
// Each case sends the payload its `.harn` emitter sends today and asserts the
// boundary loses nothing. The direction control above keeps these honest: the
// reporter still fires for a key no field reads, so silence here is a repaired
// emitter rather than a disabled census.
// ---------------------------------------------------------------------------
...
fn an_unreported_provider_error_says_so_rather_than_reading_as_no_error() {
    // An empty render must not produce "archived 7 messages after " and leave a
    // reader to decide whether the recovery had a cause.
    match accepted_host_event(
        "fix-4",
...
fn the_blank_name_receipt_keeps_both_counts() {
    ...
    match accepted_host_event("fix-5b", "tool_call_blank_name_dropped", &payload) {
        AgentEvent::FeedbackInjected { content, .. } => {
            assert!(content.contains('2') && content.contains('1'), "{content}");
        }
        other => panic!("expected FeedbackInjected, got {other:?}"),
    }
}
