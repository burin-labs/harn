//! RAII guards for workflow-scoped mutation-session and policy state.
...
pub(super) struct WorkflowExecutionPolicyGuard(pub(super) bool);

impl Drop for WorkflowExecutionPolicyGuard {
    fn drop(&mut self) {
        if self.0 {
