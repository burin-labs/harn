...
struct StopHandoffSource {
    ...
    child_run_path: Option<String>,
    execution: Option<WorkerExecutionProfile>,
    workspace_anchor: Option<crate::workspace_anchor::WorkspaceAnchor>,
    token_budget: Option<i64>,
    latest_payload: Option<serde_json::Value>,
    latest_error: Option<String>,
}

struct StopHandoffTree {
    payload: serde_json::Value,
    handoffs: Vec<serde_json::Value>,
    descendant_worker_payloads: Vec<(String, serde_json::Value)>,
}

fn parse_stop_options(args: &[VmValue], caller: &str) -> Result<WorkerStopOptions, VmError> {
...
fn worker_session_id(worker: &WorkerState) -> Option<String> {
...
fn worker_parent_session_id(worker: &WorkerState) -> Option<String> {
    match &worker.config {
...
fn worker_state_for_session_id(session_id: &str) -> Option<Arc<parking_lot::Mutex<WorkerState>>> {
    WORKER_REGISTRY.with(|registry| {
...
fn vm_value_i64(value: &VmValue) -> Option<i64> {
    match value {
...
fn vm_options_token_budget(options: &BTreeMap<String, VmValue>) -> Option<i64> {
    options
        .get("token_budget")
...
fn source_token_budget_from_worker_config(config: &WorkerConfig) -> Option<i64> {
    match config {
...
fn transcript_tokens_used_for_handoff(transcript: &VmValue) -> i64 {
    transcript
        .as_dict()
...
fn source_from_worker(worker: &WorkerState) -> StopHandoffSource {
...
fn source_from_session(session_id: &str) -> StopHandoffSource {
    StopHandoffSource {
        worker_id: None,
...
fn source_for_session(session_id: &str) -> StopHandoffSource {
...
fn source_label(source: &StopHandoffSource) -> String {
...
fn transcript_content_text(value: &VmValue) -> Option<String> {
    match value {
        VmValue::String(text) => {
            let text = text.trim();
...
fn transcript_summary_for_handoff(transcript: Option<&VmValue>) -> Option<String> {
    let dict = transcript?.as_dict()?;
    if let Some(summary) = crate::llm::helpers::transcript_summary_text(dict) {
        let summary = summary.trim();
        if !summary.is_empty() {
...
fn latest_payload_summary(payload: Option<&serde_json::Value>) -> Option<String> {
    let payload = payload?;
...
fn artifact_touched_entities(artifacts: &[ArtifactRecord]) -> Vec<String> {
...
fn evidence_refs_for_source(source: &StopHandoffSource) -> Vec<serde_json::Value> {
...
async fn finish_worker_with_event(
    ctx: &AsyncBuiltinCtx,
    state: Arc<parking_lot::Mutex<WorkerState>>,
    status: &'static str,
    event: WorkerEvent,
...
async fn stop_descendant_workers(
    ctx: &AsyncBuiltinCtx,
