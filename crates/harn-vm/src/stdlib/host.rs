...
thread_local! {
    ...
    // `host_has` reports on the capability manifest, so a fixtured operation
    // has to appear there or a script that gates its host call on `host_has`
    // never reaches the fixture. The retired `with_host_mocks` wrapper got this
    // for free through HOST_MOCKS; capability fixtures need the same merge.
...
fn fixtured_host_operations() -> Vec<(String, String)> {
...
fn capability_manifest_with_mocks() -> VmValue {
    ...
    for (capability, operation) in fixtured_host_operations() {
        ensure_mocked_capability(&mut root, &capability, &operation);
    }
    VmValue::dict(root)
}
