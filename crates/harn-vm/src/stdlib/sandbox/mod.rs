...
//! * **Windows** ([`windows::Backend`]): a write-restricted copy of the
//!   caller's token plus a Job Object, launched through
//!   `CreateProcessAsUserW`. Reads are the user's own; a write succeeds
//!   only where one of the token's restricting SIDs (Everyone, the logon
//!   session, and one SID per policy) may write, and the only grants this
//!   backend makes are Modify entries for the policy SID on the policy's
//!   writable roots. Reads and network are not confined on Windows.
...
// The Windows backend confines writes only, so it never asks whether a
// policy allows network.
