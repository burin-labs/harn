...
fn dispatched_text_channel_result_omits_absent_data() {
    ...
    assert!(last["_harn"].get("data").is_none());
}
...
fn dispatched_result_projects_mutation_facts_into_typed_transcript_receipt() {
    ...
    let last = vm_to_json(messages.last().expect("a recorded result message"));
    assert_eq!(last["_harn"]["data"]["endpoint"], "GET /health");
    assert_eq!(last["_harn"]["data"]["mutation_status"], "applied");
    assert_eq!(
        last["_harn"]["data"]["changed_paths"],
        serde_json::json!([
            "src/internal-api/handlers/health.ts",
            "src/internal-api/routes.ts"
        ])
...
fn dispatched_dynamic_result_projects_nested_mutation_facts_into_typed_receipt() {
    ...
    let last = vm_to_json(messages.last().expect("a recorded result message"));
    assert_eq!(last["_harn"]["data"]["mutation_status"], "applied");
    assert_eq!(last["_harn"]["data"]["worker"], "Payment");
    assert_eq!(
        last["_harn"]["data"]["changed_paths"],
        serde_json::json!(["workers/payment.rivet", "src/payment.rv"])
    );
    assert_eq!(
        last["_harn"]["data"]["verification"],
        serde_json::json!({
            "schema": "harn.agent_tool_postcondition.v1",
            "status": "passed",
            "verified_paths": ["workers/payment.rivet", "src/payment.rv"]
...
fn dispatched_idempotent_result_preserves_satisfied_unchanged_outcome() {
    ...
    let last = vm_to_json(messages.last().expect("a recorded result message"));
    assert_eq!(last["_harn"]["data"]["mutation_status"], "unchanged");
}
