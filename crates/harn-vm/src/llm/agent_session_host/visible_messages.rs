...
///
/// Under the default placement, directive bodies remain lifecycle events in
/// durable history and occupy one fixed trailing user slot only in this
/// projection. Under append-only placement the envelope has already been
/// committed to durable history at the turn boundary, so this projection adds
/// only directives no committed message carries yet.
#[harn_builtin(
    exposure = "runtime_internal",
    effects = [],
    sig = "__host_agent_session_visible_messages(session_id: string, messages?: list|nil, append_only?: bool|nil) -> list",
    category = "agent.host",
    runtime_only = true
)]
fn host_agent_session_visible_messages_builtin(
    ...
    let projected =
        if crate::llm::helpers::append_only_directives_enabled(optional_bool(args.get(2))) {
            crate::llm::helpers::apply_append_only_reminder_messages(messages, &rendered)
        } else {
            crate::llm::helpers::apply_rendered_reminder_messages(messages, &rendered)
        };
    Ok(super::json_to_vm(&serde_json::Value::Array(projected)))
}
...
///
/// Returns the number of directives committed, which is zero when the gate is
/// off, when nothing is pending, or when everything pending is already in
/// history.
#[harn_builtin(
    exposure = "runtime_internal",
    effects = [],
    sig = "__host_agent_session_commit_directives(session_id: string, append_only?: bool|nil) -> int",
    category = "agent.host",
    runtime_only = true
)]
fn host_agent_session_commit_directives_builtin(
    ...
    if session_id.is_empty()
        || !crate::llm::helpers::append_only_directives_enabled(optional_bool(args.get(1)))
    {
...
fn optional_bool(value: Option<&VmValue>) -> Option<bool> {
    match value {
