...
impl VmLifecycleHookInvocation {
    ...
    pub async fn resolve(&self, vm: &mut crate::vm::Vm) -> Result<Arc<VmClosure>, VmError> {
        vm.resolve_callable(&self.callable).await.map_err(|error| {
            VmError::Runtime(format!(
                "failed to resolve lifecycle hook '{}': {error}",
                self.handler_name
            ))
        })
    }
...
pub fn register_vm_hook(
    ...
    // `VmClosure::retained_for_host_registry`. The lazy handler path resolves
    // and retains the module export set on first fire in the firing VM's
    // execution-root cache instead.
...
/// Register a manifest hook whose handler closure is resolved on first fire.
pub fn register_vm_hook_lazy(
    event: HookEvent,
    pattern: impl Into<String>,
    handler_name: impl Into<String>,
    lazy: crate::value::LazyVmCallable,
) {
    RUNTIME_HOOKS.with(|hooks| {
        hooks.borrow_mut().push(RuntimeHook {
            event,
            ...
            handler: RuntimeHookHandler::Vm {
                handler_name: handler_name.into(),
                callable: crate::value::VmCallable::Lazy(lazy),
            },
        });
    });
}
...
pub async fn run_lifecycle_hooks_with_control_with_ctx(
    ...
    for registration in registrations {
        ...
        let raw = vm.call_closure_pub(&closure, &[harness, arg]).await?;
        if let Some(ctx) = ctx {
