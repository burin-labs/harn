...
use harn_mcp_compat::fake_server::{spawn_fake_http_server, FakeServerBehavior};
...
async fn spawn_then_tools_then_call_completes_against_fake_stable_server() {
    ...
    let id = mcp_host::spawn(spec_for("stable", &url), mcp_host::SpawnOptions::default())
        .await
        .expect("spawn should succeed against the fake server");
    assert_eq!(id, "stable");

    let tools = mcp_host::tools("stable").await.expect("tools should list");
    ...
    let result = mcp_host::call("stable", "echo", json!({"message": "hi"}))
        .await
    ...
    mcp_host::stop("stable").expect("stop should succeed");
...
async fn discover_returns_tools_across_multiple_servers() {
    let _lock = global_test_lock().await;
    reset_thread_local_state();
    let server_a = spawn_fake_http_server(FakeServerBehavior::StableSuccess).await;
    let server_b = spawn_fake_http_server(FakeServerBehavior::StableSuccess).await;
