//! Consolidated integration-test binary for `harn-mcp-compat`.
//!
//! Each former `tests/<name>.rs` integration file is now a submodule here
//! (`tests/harn_mcp_compat/<name>.rs`). Collapsing the crate's separate integration
//! binaries into one cuts total link time and shrinks the `cargo nextest`
//! archive. Cargo auto-discovers `tests/harn_mcp_compat/main.rs` as a single integration
//! target named `harn_mcp_compat`; files under `tests/harn_mcp_compat/` are not built as separate
