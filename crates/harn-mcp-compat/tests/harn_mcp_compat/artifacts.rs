...
//!    `spec/protocol-artifacts/fixtures/mcp/` parses, has the
...
use harn_mcp_compat::fixtures::{all_fixtures, load_named, WireFixtureKind};
use harn_mcp_compat::recursive_schema::{
...
fn every_published_fixture_loads_with_required_metadata() {
    ...
    assert!(
        fixtures.len() >= 6,
        "expected at least six stable fixtures, got {}",
        fixtures.len()
    );
    for fixture in fixtures {
        ...
        assert!(
            fixture.name.starts_with("harn.mcp."),
            "fixture name {} must use the harn.mcp.* namespace",
            fixture.name
        );
    }
}
...
#[test]
fn stable_success_fixture_has_three_round_trips() {
    ...
    assert_eq!(
        ...
        6,
        "stable_success must cover discover + list + call"
    );
