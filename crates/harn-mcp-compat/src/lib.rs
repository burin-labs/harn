//! MCP 2026-07-28 stable compatibility harness.
//!
...
//!
//! - [`fake_server`] spins up an in-process stable MCP server (HTTP or stdio)
//!   that intentionally covers stable success, `server/discover`, cache hints,
...
//!   same wire) with stable request sequences. Used to validate **server**
//!   behavior end-to-end.
//! - [`fixtures`] is the checked-in wire vocabulary every test loads
//!   from. The same JSON is exported under
//!   `spec/protocol-artifacts/fixtures/mcp/` so downstream hosts and
...
pub const FIXTURE_ROOT: &str = "spec/protocol-artifacts/fixtures/mcp";
