...
#[test]
fn test_parameter_default_resolves_before_current_parameter_binding() {
    let errs = errors(
...
const read: fn(int) -> int = fn(value = value) -> int { value }"#,
    ...
    assert!(
        closure_errs
            .iter()
            .any(|err| err.contains("expected int") && err.contains("found string")),
        "contextual closure defaults must use the same lookup order: {closure_errs:?}"
    );
  ...
  fn read(value: int = value) -> int { return value }
    ...
    assert!(
        nested_fn_errs
            .iter()
            .any(|err| err.contains("expected int") && err.contains("found string")),
        "nested function defaults must resolve in their declaration scope: {nested_fn_errs:?}"
    );

    let nested_tool_errs = errors(
        r#"pipeline default(task) {
  const value: string = "outer"
  tool read(value: int = value) -> int { return value }
    ...
    assert!(
        nested_tool_errs
            .iter()
            .any(|err| err.contains("expected int") && err.contains("found string")),
        "nested tool defaults must resolve in their declaration scope: {nested_tool_errs:?}"
    );

    let nested_body_errs = errors(
        r#"pipeline default(task) {
  const value: string = "outer"
  fn read() -> int { return value }
    ...
    assert!(
        nested_body_errs
            .iter()
            .any(|err| err.contains("expected int") && err.contains("found string")),
        "nested callable bodies must resolve in their declaration scope: {nested_body_errs:?}"
    );
}
...
fn test_duplicate_pipeline_enums_shadow_in_source_order() {
  ...
  enum Event { Second(value: int) }
    ...
    assert!(
        errs.is_empty(),
        "pipeline enum declarations did not shadow in source order: {errs:?}"
    );
}
...
pipeline default(task) extends base {
  ...
  enum Event { Child(value: int) }
    ...
    assert!(
        errs.is_empty(),
        "parent enum declaration changed the child catalog: {errs:?}"
    );
}
