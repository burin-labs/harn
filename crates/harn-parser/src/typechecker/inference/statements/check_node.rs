    ...
    pub(in crate::typechecker) fn check_node(&mut self, snode: &SNode, scope: &mut TypeScope) {
        ...
        match &snode.node {
            ...
            Node::Assignment {
                ...
                self.check_assignment(target, value, op, span, scope);
            }
            ...
            Node::MethodCall {
                ...
                // `EnumName.Variant(args)` is parsed as MethodCall. Route it
                // through the enum-construct checker so payload field types
                // (including opaque host names) are enforced — the same
                // contract the unused `EnumConstruct` arm already had.
                if let Node::Identifier(enum_name) = &object.node {
                    if scope.get_enum(enum_name).is_some() {
                        self.check_enum_variant_construct(enum_name, method, args, span, scope);
                        return;
                    }
                }
            ...
            Node::EnumConstruct {
                ...
                self.check_enum_variant_construct(enum_name, variant, args, span, scope);
            }
