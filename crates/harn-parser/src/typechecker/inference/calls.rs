...
impl TypeChecker {
    ...
    fn check_call_signature_arguments(
        ...
        for (i, arg) in args.iter().enumerate() {
            ...
            if let Some(violation) = self.literal_union_violation(arg, expected, call_scope) {
    ...
    /// homogeneous string/int-literal union parameter — the exact shape the
    /// VM lowers to an `enum` schema and enforces at runtime (see
    /// `type_expr_to_schema_value` in `harn-vm`, which builds an enum only for
    /// a non-empty union of *all* `LitString` or *all* `LitInt`). When the
    /// argument's value is already known at check time there is no reason to
    /// defer that rejection to a runtime `TypeError`, so this reproduces the
    /// VM's decision statically and only then.
    ///
    ...
    /// runtime verdicts in lockstep — no value is rejected here that the
    /// runtime would accept.
    fn literal_union_violation(
        &self,
        arg: &SNode,
        expected: &TypeExpr,
        scope: &TypeScope,
    ) -> Option<LiteralUnionViolation> {
        let resolved = self.resolve_alias(expected, scope);
