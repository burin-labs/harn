...
.PHONY: check-docs check-docs-cookbook-entrypoints
...
check-docs:
	...
	$(MAKE) --no-print-directory HARN_BIN="$$harn_bin" check-language-spec check-trigger-quickref check-provider-matrix check-provider-support check-provider-catalog check-connector-matrix check-trigger-examples check-docs-model-refs check-docs-snippets check-docs-cli-flags check-docs-links check-docs-cookbook-entrypoints check-site-snippets check-docs-workflow-quickstart check-diagnostics-catalog check-generated-registry
