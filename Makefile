...
test-pr-gate-scripts:
	...
	./scripts/tests/check_make_gate_coverage_test.sh

# Build-bearing half: each of these spawns a real cargo build, so they need the
# warm Rust toolchain and cannot sit on the fast pull-request path. Wired into
# CI through test-pr-gate-post-warm-integrations.
test-pr-gate-scripts-build:
...
test-pr-gate-post-warm-integrations: test-rust-lint-lane-cache
	...
	$(MAKE) HARN_BIN="$(HARN_BIN)" test-pr-gate-scripts-build
