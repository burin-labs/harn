- **Release binary builds no longer cancel healthy platform legs.** The
  ...
  fill only the genuinely missing assets. Recovery skip checks and release
  smoke now also require `SHA256SUMS`, so a release missing its checksum
  sidecar is not treated as complete.
