- **`make check-drift` runs the source-reading drift/manifest guards in seconds
  ...
  `make check-drift-binary` for the guards that compare against binary-emitted
  output (they need a freshly built binary); `make all` remains the full gate.
