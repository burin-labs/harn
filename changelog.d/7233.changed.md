...
gone. The gate globs `crates/harn-cli/assets/demo/*/scenario.harn`, the same
directory `gallery.ts` imports from, runs `lint --strict
--require-public-api-types` alongside `check`, and fails if either glob comes
back empty. Coverage went from 4 files to 25.
