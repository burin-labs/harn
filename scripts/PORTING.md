...
| `check_binary_size.harn` | `build-release-binaries.yml` (invokes the just-built linux-x64 release binary) | Two-level size policy (distribution fuse + growth signal) + cargo-bloat report; parity-verified on `--no-build` pass/fail/usage/missing-bloat paths. Sandbox note: out-of-tree HARN_BIN/CARGO_TARGET_DIR needs `--no-sandbox`. |
...
| `check_docs_workflow_quickstart.harn` | `make check-docs-workflow-quickstart` | Pinned digest / executed-node / connector-shape assertions via `harness.process.run` (+`cwd` for the connect demo); parity-verified incl. digest-drift failure. |
