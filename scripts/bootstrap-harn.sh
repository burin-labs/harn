...
run_seed() {
  if [ "${1:-}" = --seed-command ]; then
    ...
    "$HARN_EXT_BOOTSTRAP_INSTALLER" run --standalone "$root/scripts/bootstrap-harn.harn" -- "$@"
  fi
}
...
download() {
  if command -v curl >/dev/null 2>&1; then curl -fLsS --retry 3 --connect-timeout 15 --max-time 120 "$1" -o "$2"
...
if [ ! -f "$cache/SHA256SUMS" ]; then
  [ "$HARN_EXT_BOOTSTRAP_OFFLINE" != 1 ] || { echo 'offline seed manifest missing' >&2; exit 1; }
  download "$base/SHA256SUMS" "$work/SHA256SUMS"
  mv "$work/SHA256SUMS" "$cache/SHA256SUMS"
fi
...
if [ "$downloaded" = 1 ]; then
  cp "$work/$asset" "$cache/$asset.tmp.$$"
  mv "$cache/$asset.tmp.$$" "$cache/$asset"
fi
