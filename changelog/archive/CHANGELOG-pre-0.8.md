...
scoped to the current major series. The split point and archive
filename match what `harn-bump-fleet/lib/changelog.harn::
changelog_archive_below_version(text, "0.8")` would emit, so the
v0.9.0 cut can run that helper without producing a divergent layout.
