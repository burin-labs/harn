...
- I do not have a fresh, apples-to-apples numeric baseline against current
  downstream-host pipelines for the exact same three prompts. Recent downstream
  eval work exists, but not as a clean side-by-side experiment on these
  exact tasks.
...
| Dimension | Burin Mini DFS queue | Current downstream pipeline |
| --- | --- | --- |
...
- this exact pipeline as a production replacement for a downstream host
...
- If a clean downstream baseline is needed later, run the same 3 prompts
