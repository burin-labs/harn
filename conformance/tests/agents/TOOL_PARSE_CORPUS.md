...
of Rust unit tests into executable conformance fixtures for the harn#5142
...
|---|---|---|
| `dropped` fragment granularity | `tool_parse_drops` | Whole-unit fragments instead of line slices are fine. The invariant is that *something* is recorded, not where the cuts fall. |
...
This table records compatibility decisions made during the port. Items 1 and 2
remain pinned behavior; items 3–5 were unreachable Rust paths that the Harn
composition did not resurrect; item 6 is not a real conflict.
...
These behaviors are deliberately outside this corpus. Their focused Rust
coverage remains alongside the owning native/normalization/transport modules.

| Rust test(s) | Reason | Substitute here |
|---|---|---|
| `native_tools.rs` | Native tool channel, not a text dialect. | none needed |
| validation, normalization, assistant-message, and local-read tests | Downstream or provider-native behavior. Consolidated in `tools/tests/non_dialect.rs`. | The parser/normalizer split is also pinned by `tool_parse_markup`. |
| `wire_to_canonical` behavior | Transport-layer remap, not exposed through a public Harn surface. | Unit and streaming parity coverage lives in `tool_delimiter.rs` and `providers/openai_compat.rs`; `tool_parse_reserved` covers the parser-owned malformed opener. |
