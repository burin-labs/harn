...
| `agent_loop(prompt, system?, options?)` | prompt: string, system: string, options: dict | dict | Multi-turn agent loop with `##DONE##` completion sentinel (`<done>##DONE##</done>` in tagged text-tool stages), daemon/idling support, and optional per-turn context filtering. Returns `{status, text, iterations, duration_ms, tools_used}` |
