...
System reminders are typed transcript injections for a running agent session.
They let Harn add ambient context at turn boundaries, such as token-pressure
warnings, idle-session nudges, truncated-tool-output notices, post-compact
recaps, host file-change alerts, and runtime corrective feedback, without a
producer having to place a turn itself. Producers work in lifecycle terms —
body, tags, TTL, authority — and the runtime decides when a directive is
delivered and where it lands. Delivery appends one durable turn at the emitting
turn's boundary; see [append-only placement](#append-only-placement).
...
|---|---|---|
| `interrupt_immediate` | `iteration_start`, `pre_tool_dispatch`, `post_tool_dispatch`, `iteration_end`, `daemon_idle_pre`, `daemon_idle_post` | When it arrives at `pre_tool_dispatch` (between LLM return and tool dispatch), the pending tool batch is **skipped** and the reminder lands in the next iteration's prompt. At every other seam, the reminder is appended to the transcript and visible on the next prompt build. |
| `finish_step` | `iteration_start`, `post_tool_dispatch`, `iteration_end` | Drained at iteration boundaries only. Never short-circuits a tool batch. The model sees this reminder on the *next* prompt build — including the final iteration when the loop is about to terminate. |
...
5. the envelope becomes one new trailing user turn. It never folds into an
   existing message, whatever role the conversation ends in.
...
chains one hash per KV block over the tokens before it, and a llama.cpp slot
re-prefills from the first divergent token to the end. A directive that moves
between requests therefore invalidates the whole prompt on every turn.

Transcript construction holds one invariant:
...
it, and later turns re-send those exact bytes at the same index:
...
Two consequences follow. A directive stays visible for the rest of the session
rather than expiring at its `ttl_turns` boundary, because withdrawing it would
...
The pending queue lives in transcript events; a delivered directive is a durable
message. TTL governs the queue, so it decides how long a directive keeps being
*re-issued*, not how long a delivered one stays visible. Normal agent-session
post-turn processing decrements finite `ttl_turns`. `ttl_turns: 1` expires at
