...
Spend, token, and concurrency ceilings are not part of this document. A run
enforces the typed session budget (`budget.total_budget_usd` / `harness.llm.budget`),
not a config-file or `HARN_BUDGET_*` overlay. A file that still carries
`[limits]`, `[permissions]`, or `[policy]` fails validation by name.
