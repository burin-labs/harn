...
`connector_call` throws `{error: "connector_error", kind, message}` when an
active connector rejects a call. `kind` is one of `method_not_found`,
`invalid_args`, `rate_limited`, `transport`, or `other`. Destination-policy
failures keep the more specific `EgressBlocked` record described above.
