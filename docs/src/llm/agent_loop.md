...
| `tool_caller` with `with_structural_validator(...)` | caller/dict | nil | Deterministic pre-dispatch validation for assistant turns that should have produced tool progress. In SV-1, `with_structural_validator({on_failure?: "regenerate_with_feedback" \| "raise"})` rejects zero-tool-call turns when write-capable tools are in scope and no done marker was emitted. Rejects emit `structural_validator_decision`, then either pop+regen with `structural_validator` feedback or raise immediately |
