...
| `command_behavior` / `run_command_behavior` | Defaults for every generated `run_command` request. `background_after_ms` gives the command a short foreground startup window, then returns `{status: "running", stdout, stderr, output_path, ...}` while progress events continue in the session. `progress_interval_ms` seeds the feedback cadence: it is the delay before the first progress snapshot and then doubles after each one up to an internal ceiling (default 30000ms), so a long-running command emits frequent early progress that thins out over time. `progress_max_inline_bytes` caps snippets. |
