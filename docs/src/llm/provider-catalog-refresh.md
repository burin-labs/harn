...
harn provider catalog validate --check-artifacts
...
`make gen-provider-catalog` runs `harn provider catalog build-config` followed
by `harn provider catalog export`, and `make check-provider-catalog` checks both
...
`harn provider catalog export --overlay <path>`. Overlays are merged with the
...
Both return the same provider catalog v4 artifact shape generated by
`harn provider catalog export`: `schema_version`, `schema`, `generated_by`,
`providers`, `models`, `aliases`, `variants`, `routing_routes`, and
`qc_defaults`. The
...
`endpoint.regions`), and pricing without shipping their own model/provider
tables.
