...
Workflow crystallization mines repeated agent traces into a reviewable workflow
skeleton and a machine-readable manifest of what that workflow would need to
run. It is a review loop, not a code generator:
...
3. Generate a workflow skeleton, replay-gated skill candidates, and a
...
6. Write the step bodies, then package or promote the approved workflow so
   later runs use CPU/interpreter steps for the stable portion and reserve
   model calls for ambiguity.

## What the generated workflow is

Read this before you plan around the output.

The `.harn` file `harn crystallize` writes is an **outline**. Its executable
body logs one line per mined step, accumulates a warning for each step that
still needs a model or a reviewer, and returns `status: "shadow_ready"`. It
does not check out the branch, write the file, or call the tool that the
original trace did. Step 6 above — writing the bodies — is a human's job.

What the miner actually establishes, and what makes the artifact worth
reviewing, is everything around that body:

- the parameters it extracted from fields that varied across the source traces
- the capability set and required secrets, in the file's header comment
- the side effect each step requested, and where an approval boundary sits
- which steps are deterministic and which are fuzzy
- the report, eval pack, and shadow comparison described below

So the honest summary is: crystallization tells you what a repeated workflow
does, what it touches, and what it would need to be trusted. It does not write
...
This is the complete output of mining the checked-in three-trace version-bump
fixture, unedited:

```harn,check
/**
 * Generated by harn crystallize. Review before promotion.
 * Candidate: candidate_92144cef9e215de6
 * Source trace hashes: sha256:49eab89d…, sha256:6932385b…,
 sha256:ecf9e1ce…
...
pipeline crystallized_version_bump(
  ...
  // Step 2: file_mutation update_manifest_version
  // side_effect: file_write harn.toml
...
Every line that does anything is a log call. The branch is not checked out,
`harn.toml` is not written, `cargo package` is not run. The six parameters were
extracted from the fields that varied across the three source traces, and the
header records the three capabilities those steps would need.

A fuzzy step — one whose original action depended on a model — adds an approval
boundary, a review comment, and a warning that the workflow carries out of the
run:

```harn,ignore
  // Step 15: agent_review agent_review[release_audit]
  // approval_required: release_audit_review
  // review_required: fuzzy segment still
  // needs LLM/reviewer handling before
  // deterministic promotion.
  review_warnings = review_warnings.appending(
    "fuzzy step: agent_review[release_audit]"
  )
  harness.obs.log("crystallized step 15: agent_review[release_audit]")
```

When no safe candidate is found the file is a four-line stub that logs
`"no safe crystallization candidate"` and returns `status: "rejected"`.
...
pipeline capture(harness: Harness, report) {
  return composition_crystallization_input(
    report, {id: "composition-trace"},
  )
}
```
