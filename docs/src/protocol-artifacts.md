...
- `schemas/*.schema.json`: stable copies of the ACP, A2A, MCP, tool-receipt,
  collaborative plan-document, and `harn-tools/1.0` contracts covered by
...
- `harn-tools.ts`: strict TypeScript types for the transport-neutral tool
  ...
  `schemas/harn-tools-v1.schema.json` from the same Rust contract types.
...
This release formalizes the existing `harn-tools/1.0` runtime shape as its
first generated contract. After that contract ships, its discriminator versions
the tool catalog independently of the Harn crate. Additive optional fields keep
the discriminator. Removing a field, changing a field type, or tightening an
owned closed record requires a new catalog discriminator and a migration note.
...
`schemas/harn-tools-v1.schema.json`; they should not infer catalog types from
