...
```harn
const t = transcript()
const result = agent_loop(harness, "task", nil, {transcript: t, provider: "mock"})
```
...
const result = agent_loop(harness, "task", nil, {session_id: sid, provider: "mock"})
// `harness.agent.snapshot(sid)` if you want the transcript back as a dict.
```
