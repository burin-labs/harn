...
// model_policy:
const sid = agent_session_open("summarize-v2")
...
const sid = agent_session_open("chat-42")
const result = agent_loop("hi", nil, {session_id: sid})
```
...
```harn
const t = transcript()
const result = agent_loop("task", nil, {transcript: t, provider: "mock"})
```

### After

```harn
const sid = agent_session_open()
const result = agent_loop("task", nil, {session_id: sid, provider: "mock"})
