...
Get the destination from `harn contracts builtins`, which lists every declared
`harness.<capability>.<method>` with its handle type. Underscored capability
names (`pr_monitor` for a `prmonitor.*` target) are the usual spelling
difference. Arguments do not carry over unchanged: `host_call` took a params
dict, and a typed method takes the parameters its signature declares.
