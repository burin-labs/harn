...
Two open defects make an unchecked bulk apply unsafe: [#6148] places an
inserted argument at a stale byte offset once a file takes more than one
repair — which is the normal case in a file with several call sites — and
[#6149] over-attenuates connector runtime exports. Both produce source that
looks plausible in a diff. Applying per file, or in small batches, keeps the
