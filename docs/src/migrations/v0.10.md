...
`top_p`, `top_k`, `seed`, `frequency_penalty`, `presence_penalty`, `stop`,
`cache: true`, and `prompt_cache_ttl` are admitted before provider transport.
An explicit incompatibility throws a terminal `invalid_request` error.
...
control below `provider_options.<provider>`. Catalog defaults do not count as
caller intent. Unknown custom generation routes remain open-world, but custom
prompt-cache routes must declare `prompt_caching` and any selectable
`prompt_cache_ttls` in `harn.toml` because their wire lowering is
provider-specific.
