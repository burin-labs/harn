...
```harn,ignore
let r = llm_call("prompt", nil, {llm_retries: 2, llm_backoff_ms: 500})
```
...
let caller = with_retry(default_llm_caller(), {max_attempts: 3, base_ms: 500})
let envelope = caller({prompt: "prompt", system: nil, opts: {}})
```
...
let stack = agent_stack({role: "planner", retry: {max_attempts: 3}, required_reason: true})
...
let route = agent_model_options({role: "planner"})
let mw = with_required_reason({})
let result = agent_loop(task, system, route.options + {
  loop_until_done: true,
  llm_caller: with_retry(default_llm_caller(), {max_attempts: 3}),
...
let result = audit_agent("Audit the release", {tools: tools})
```
...
let opts = agent_preset("audit", {tools: tools})
let result = agent_loop("Audit the release", opts?.system, opts)
```
...
```harn,ignore
let opts = agent_preset("repair", {iteration_budget: {mode: "adaptive", initial: 4, max: 24, extend_by: 2}})
```
